...
const MAX_SHOWCASED_ACHIEVEMENTS = 3;

function parseShowcasedAchievementIds(rawValue: unknown): number[] {
...
app.post("/api/profile/customization", authMiddleware, async (c) => {
  ...
  const showcasedAchievementIds = Array.isArray(body.showcased_achievements)
    ? Array.from(
        new Set(
          body.showcased_achievements
            .map((value) => Number(value))
            .filter((value) => Number.isInteger(value) && value > 0),
        ),
      ).slice(0, MAX_SHOWCASED_ACHIEVEMENTS)
    : null;
  const showcasedAchievements = showcasedAchievementIds ? JSON.stringify(showcasedAchievementIds) : null;
...
app.post("/api/profile/achievements/showcase", authMiddleware, async (c) => {
  const user = c.get("user");
  if (!user) return c.json({ error: "Unauthorized" }, 401);
...
app.get("/api/progression", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (!progression) {
      return c.json({ error: "Progressão não encontrada", code: "PROGRESSION_NOT_FOUND" }, 404);
    }
...
app.post("/api/titles/:id/activate", authMiddleware, async (c) => {
  ...
  if (!Number.isInteger(titleId) || titleId <= 0) {
    return c.json({ error: "ID de título inválido" }, 400);
  }

  const unlockedTitle = await c.env.fitloot_db
    .prepare("SELECT id FROM user_titles WHERE user_id = ? AND title_id = ? LIMIT 1")
    .bind(user.id, titleId)
    .first<{ id: number }>();

  if (!unlockedTitle) {
    return c.json({ error: "Título não desbloqueado", code: "TITLE_NOT_UNLOCKED" }, 404);
  }

  await c.env.fitloot_db
    .prepare("UPDATE user_titles SET is_active = 0, is_equipped = 0, updated_at = datetime('now') WHERE user_id = ?")
    .bind(user.id)
    .run();

  await c.env.fitloot_db
    .prepare("UPDATE user_titles SET is_active = 1, is_equipped = 1, updated_at = datetime('now') WHERE user_id = ? AND title_id = ?")
    .bind(user.id, titleId)
    .run();

  return c.json({ success: true, title_id: titleId });
});

app.post("/api/titles/:id/deactivate", authMiddleware, async (c) => {
  const user = c.get("user");
  if (!user) return c.json({ error: "Unauthorized" }, 401);
  ...
  await c.env.fitloot_db
    .prepare("UPDATE user_titles SET is_active = 0, is_equipped = 0, updated_at = datetime('now') WHERE user_id = ? AND title_id = ?")
    .bind(user.id, titleId)
    .run();
...
function ensureUtf8JsonCharset(response: Response): Response {
  const contentType = response.headers.get("Content-Type");
  if (!contentType) return response;

  const [mediaType] = contentType.split(";");
  if (mediaType.trim().toLowerCase() !== "application/json") return response;
...
async function handleFetchWithGuard(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  try {
    ...
    return ensureUtf8JsonCharset(response);
  } catch (error) {
