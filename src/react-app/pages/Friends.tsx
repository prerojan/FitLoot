...
export default function Friends() {
  ...
  const loadFriends = useCallback(async () => {
    try {
      ...
      setError("N„o foi possÌvel carregar a lista de amigos agora.");
    } finally {
      setLoading(false);
    }
  }, [navigate]);
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      setError("N„o foi possÌvel buscar usu·rios agora.");
    } finally {
      setSearching(false);
    }
  };
  ...
  const sendFriendRequest = async (friendUserId: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        setError(payload?.error || "N„o foi possÌvel enviar solicitaÁ„o de amizade.");
        return;
      }
  ...
  const acceptFriendRequest = async (requestId: number) => {
    try {
      ...
      if (!response.ok) {
        const payload = (await response.json().catch(() => null)) as { error?: string | undefined } | null;
        setError(payload?.error || "N„o foi possÌvel aceitar a solicitaÁ„o.");
        return;
      }
  ...
  const rejectFriendRequest = async (requestId: number) => {
    try {
      ...
      if (!response.ok) {
        const payload = (await response.json().catch(() => null)) as { error?: string | undefined } | null;
        setError(payload?.error || "N„o foi possÌvel recusar a solicitaÁ„o.");
        return;
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 pb-24">
      ...
      <div className="bg-white/80 backdrop-blur-sm border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-screen-xl mx-auto px-4 py-6">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Buscar usu√°rios por username..."
              value={searchQuery}
                ...
                <div key={result.user_id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Badge className="mt-1">N√≠vel {result.level}</Badge>
                    </div>
                  </div>
                  <Button
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        ...
        {pendingRequests.length > 0 && (
          <div className="mb-6">
            <h2 className="fl-title-card mb-4">Solicita√ß√µes Pendentes ({pendingRequests.length})</h2>
        ...
        {friends.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">Voc√™ ainda n√£o tem amigos adicionados</p>
            <p className="text-gray-400 text-sm">Use a busca acima para encontrar usu√°rios</p>
          </div>
        ) : (
