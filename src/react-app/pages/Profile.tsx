...
export default function Profile() {
  ...
  if (loading && !profile && !attributes && !progression) return <AppPageShell bottomNavActive="profile" className="bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50" profile={profile} progression={progression}><div className="fl-app-container space-y-4 py-6 sm:py-10"><div className="fl-card flex items-center justify-center p-6"><LoadingBall size="md" /></div><div className="fl-card flex items-center justify-center p-6"><LoadingBall size="sm" /></div></div></AppPageShell>;
  if (error) return <AppPageShell bottomNavActive="profile" className="bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50" profile={profile} progression={progression}><div className="fl-app-container py-10 text-center sm:py-12"><p className="mb-4 text-red-600">{error}</p><button onClick={retry} className="fl-btn-primary rounded-xl px-4 py-2">Tentar novamente</button></div></AppPageShell>;

  return (
    <AppPageShell bottomNavActive="profile" className="bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50" profile={profile} progression={progression}>
